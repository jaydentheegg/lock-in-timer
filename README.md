...
25 分钟进入 DEEP 时，用 1.5 秒将当前白色星尘背景推进、像素化、向两侧展开，最终留下黑底与局部城市视频碎片。3D 版本只采样独立的背景粒子层，CSS 版本使用二维星尘；不移动时间数字，不中断音乐或修改累计时间。进入后显示一行 `DEEP / 已进入深层`，红橙故障结束后清除。

开场为低亮度白色星尘，拖动黑银工牌后进入探索。工牌印有 `Lock-In Timer`，拥有独立正反面、薄卡体和金属扣件。LOCK-IN 入口的可见粒子字缩为原来的 70%，点击区域不变。旧螺旋图片文件保留，但不再加载或显示。
...
本地无 WebGL 预览可直接打开 `/fallback.html?preview=events`，无需改变浏览器或系统设置。线上构建仍保留原 `/fallback/` 地址。减少动态效果下背景星点静止，不漂移、不排斥鼠标。
...
- 白色星尘使用独立的渲染层，转场只采样该层；时钟用离屏字形采样画成粒子，按字位分槽，没变的那一位不动。鼠标对背景星尘产生轻微排斥。
